...
RingBuffer::RingBuffer(int capacity) noexcept :
...
int RingBuffer::Enqueue(const char* src, int size) noexcept {
	...
	int firstChunk = min(size, DirectEnqueueSize());
...
int SendRingBuffer::SendTCP(SOCKET socket) noexcept {
...
int RecvRingBuffer::RecvTCP(SOCKET socket) noexcept {
